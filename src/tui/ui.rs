...
pub fn draw(f: &mut Frame, app: &App) {
    ...
    draw_transfers_panel(
        ...
        &app.web_uploads,
        TransferPanelCtx {
            focused: app.focus == Focus::Transfers,
            cursor: app.transfer_cursor,
            speed_unit: app.speed_unit,
            th,
        },
        root[2],
    );
...
fn draw_peer_files(f: &mut Frame, app: &App, th: &Theme, area: Rect) {
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(i, file)| {
            ...
            let line = Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(
                    format!("{}{}{}", &file.name, avail_marker, dl_marker),
                    name_style,
                ),
                Span::styled(
    ...
    let list = List::new(if app.peer_files.is_empty() {
        no_peer_msg
    } else {
        items
    });
...
fn draw_transfers_panel(
    ...
    let hint = if ctx.focused && !downloads.is_empty() {
        " [p] pause  [c] cancel  [↕] select"
    } else {
        ""
    };
...
fn draw_transfer_row_download(
    ...
    let icon_color = if dl.cancelled {
        th.error
    } else if dl.paused {
        th.dim
    } else if dl.retrying {
    ...
    let right_label = if dl.cancelled {
        "cancelled".to_string()
    } else if dl.paused {
        "paused".to_string()
    } else if dl.retrying {
        ...
        crate::client::format_speed_unit(dl.speed_bps, speed_unit)
    };
    ...
    let text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{} ", right_label),
                Style::default().fg(if dl.cancelled {
                    th.error
                } else if dl.retrying {
                    th.warn
                } else {
            ...
            Span::styled(
                if !dl.done && !dl.cancelled {
                    format!("{} ", format_eta(dl.eta_seconds))
                } else {
                    String::new()
                },
                Style::default().fg(th.dim),
    ...
    f.render_widget(
        Paragraph::new(text).style(Style::default().bg(sel_bg)),
        area,
    );
}

fn draw_transfer_row_upload(
    f: &mut Frame,
    ul: &UploadState,
    area: Rect,
    speed_unit: SpeedUnit,
    ...
    let bar: String = "█".repeat(filled) + &"░".repeat(bar_width - filled);
    let right_label = if ul.cancelled {
        "cancelled".to_string()
    } else if ul.done {
        "done".to_string()
    } else {
        crate::client::format_speed_unit(ul.speed_bps, speed_unit)
    };
    let text = vec![
        Line::from(vec![
            Span::styled(
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{} ", right_label),
                Style::default().fg(if ul.cancelled { th.error } else { th.dim }),
            ),
            Span::styled(
                if !ul.done && !ul.cancelled {
                    format!("{} ", format_eta(ul.eta_seconds))
                } else {
                    String::new()
                },
                Style::default().fg(th.dim),
...
fn draw_transfer_row_web_upload(
    f: &mut Frame,
    wu: &WebUploadState,
    area: Rect,
    speed_unit: SpeedUnit,
    ...
    let stats_line = if wu.total > 0 {
        Line::from(vec![
            ...
            Span::styled(
                format!("{} ", format_eta(wu.eta_seconds)),
                Style::default().fg(th.dim),
            ),
        ])
    } else {
        Line::from(vec![
            Span::styled(
                format!(
                    " {} received ",
                    crate::shares::human_size(wu.bytes_received)
                ),
                Style::default().fg(th.text),
            ),
            Span::styled(format!("{} ", status_label), Style::default().fg(th.dim)),
